...
Frontend (template renderer + static assets)
1. Start frontend server (optional):
...
RESP=$(curl -s -X POST http://localhost:8080/auth/login -H "Content-Type: application/json" -d '{"email":"teacher@example.com","password":"secret123"}')
TOKEN=$(echo "$RESP" | jq -r '.token')
# fallback:
# TOKEN=$(echo "$RESP" | sed -n 's/.*"token":"\([^"]*\)".*/\1/p')

3) Create a group
RESP=$(curl -s -X POST http://localhost:8080/groups \
...
RESP=$(curl -s -X POST http://localhost:8080/tests \
  -H "Content-Type: application/json" -H "Authorization: Bearer $TOKEN" \
...
RESP=$(curl -s -X POST http://localhost:8080/sections \
  -H "Content-Type: application/json" -H "Authorization: Bearer $TOKEN" \
...
# MCQ / short-answer
RESP=$(curl -s -X POST http://localhost:8080/items \
  -H "Content-Type: application/json" -H "Authorization: Bearer $TOKEN" \
...
curl -s -X POST http://localhost:8080/auth/register -H "Content-Type: application/json" -d '{"email":"student@example.com","password":"studpass","role":"student"}' | jq .
RESP=$(curl -s -X POST http://localhost:8080/auth/login -H "Content-Type: application/json" -d '{"email":"student@example.com","password":"studpass"}')
...
ANS_ID=$(curl -s -X POST http://localhost:8080/answers \
  -H "Content-Type: application/json" -H "Authorization: Bearer $STUDENT_TOKEN" \
  -d "{
    \"user_id\": $STUDENT_ID,
    \"test_code\": \"$TEST_CODE\",
...
curl -s -X GET http://localhost:8080/answers/$ANS_ID -H "Authorization: Bearer $TOKEN" | jq .

10) Teacher results aggregation
# aggregated results with filters: student (partial email), group, test, pass threshold
curl -s -X GET "http://localhost:8080/answers/results?group=$GROUP_CODE&pass=60" -H "Authorization: Bearer $TOKEN" | jq .

11) Dashboard
curl -s -X GET http://localhost:8080/dashboard -H "Authorization: Bearer $TOKEN" | jq .

Other useful endpoints
- List tests: GET /tests?name=&group=
  curl -s -X GET "http://localhost:8080/tests?group=$GROUP_CODE" -H "Authorization: Bearer $TOKEN" | jq .
- Group detail (members, quizzes, avg performance): GET /groups/:code/detail
  curl -s -X GET http://localhost:8080/groups/$GROUP_CODE/detail -H "Authorization: Bearer $TOKEN" | jq .
...
- If server port differs, change :8080 accordingly.
