...
function createTest({ code, name, group_code, owner_id }) {
  const db = getDB();
  const c = code || genCode();
  db.prepare('INSERT INTO tests (code, name, owner_id, group_code) VALUES (?, ?, ?, ?)').run(c, name, owner_id, group_code);
  return { code: c, name, owner_id, group_code };
}

function getTest(code) {
  const db = getDB();
  return db.prepare('SELECT t.code, t.name, t.group_code, t.owner_id, t.created_at, t.updated_at, g.name as group_name FROM tests t LEFT JOIN groups g ON g.code = t.group_code WHERE t.code = ?').get(code);
}
...
function listTests({ name, group_code, owner_id } = {}) {
  const db = getDB();
  let q = 'SELECT code, name, group_code, owner_id FROM tests WHERE 1=1';
  const params = [];
